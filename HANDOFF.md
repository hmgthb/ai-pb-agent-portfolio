...
⚠️ **아직 라이브로 안 돌린 것**: 새 유니버스로 F2를 실행하지 않았다(크레딧). 현재 최신 브리프는
이전 워치리스트(삼성·하이닉스·현대차)이고 `market`이 비어 있다. 데모 전에 `POST /api/briefs/run`
1회(약 40초) 실행할 것 — 그래야 카드가 고객 보유 종목으로 바뀐다.
...
[대시보드] Next.js `/dashboard` · 기능 레일(F1 카드→채팅) · 상담 전 브리핑(오늘 시장 + 고객 보유 종목)
  고객 관리 탭: 팩트 노트 생성(PB·관리자, F3 SSE 직접 구독) · 처리 대기 큐 · 고객 50 · 고객별 상담 준비 메모
