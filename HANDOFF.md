...
- **브리프 #7**(2026-07-22, 고객 보유 상위 3종목)이 최신이다. 고객 시드를 바꾸면 유니버스도 바뀌므로
  브리프를 다시 돌려야 화면과 맞는다(`POST /api/briefs/run`, 약 40초·크레딧).
- ⚠️ **데모 노트 5건과 브리프 종목이 겹치지 않는다** — 그래서 고객 상세의 상담 준비 메모는 대부분
  "오늘 브리핑·노트에 이 종목은 없습니다"로 뜬다(정직한 동작이지만 데모에서는 빈손으로 보인다).
  NAVER 보유 고객(예: 강준서)을 고르면 팩트 노트가 붙은 줄을 보여줄 수 있다 — **데모 대본에 넣을 것**.
...
[사람] 노트: 검토→심의→발행(준법, 게이트 재검사) / 고객 문의: 확인 완료(담당 PB, 회신은 사람이 직접 작성)
       — AI 개입 없음, 전 과정 audit_log
```
