...
> 화면에 보이는 고객은 **전부 가상의 합성 데이터**다 (아래 「데이터에 관한 고지」).

### ① 상담 전 브리핑

![브리핑](docs/screenshots/01-brief.png)

거시 지표 띠 아래로 **3개월 추세 2줄**과 **내 고객이 보유한 종목 2줄**이 선다. 어느 지표가
튀었는지, 어느 종목을 올릴지는 **전부 규칙이 고르고**(`brief.py`) LLM은 문장만 쓴다.
`-19.2% (14일) · 21명 보유` 같은 수치는 코드가 세어 배지로 붙이고, 문장에는 각주 `[1][2][3]`이 달린다.

### ② 고객 포트폴리오 · ③ Next Best Action

| | |
| --- | --- |
| ![고객 포트폴리오](docs/screenshots/02-portfolio.png) | ![Next Best Action](docs/screenshots/03-nba.png) |
| 담당 고객 50명, 보유·비중·자산배분·위험 플래그. 계좌번호는 마스킹되고, 종목별 금액은 **비중(%)으로 바뀐 뒤에만** 경계를 넘는다 (`redact.py`). | 상담 이력과 계좌 데이터를 근거로 답하고 **「Next Action」 절**까지 쓴다. "마지막 상담이 2개월 전이므로…"의 **개월 수는 코드가 세고**, 모델은 그 사실을 읽고 무엇을 할지만 쓴다. |

### ④ 상담 준비 메모 (PDF)

![상담 준비 메모](docs/screenshots/04-memo.png)

위 셋을 한 장으로 뽑는다. 문장마다 각주가 붙고, 출처란이 **`[1] 보유 · 계좌 보유데이터
(내부·공개데이터 아님)`**이라고 근거의 성격을 밝힌다. 페이지 하단에는
`상담 준비 메모 · 내부 참고용 · 고객 제공용 아님`이 매 장 찍힌다(`notepdf.py`).
