...
| **F2 브리핑(거시)** | 동작 | 나스닥·S&P500·원/달러·국고채10년·미국채30년 + 어제 대비 + 밤사이 헤드라인 (**새 에이전트 0개**) |
| **F1 Next Best Action** | 동작 | 상담 중 질문 → 규칙 라우팅 → 출처·지연시세 명시 답변(멀티턴) |
| **H1 대시보드** | 동작 | 브리핑 · 고객 카드(Next Best Action) · 상담 준비 메모 / (관리자) 작성·검토 · 감시 |
...
```
**입력이 없다**(2026-08-07). 브리핑이 거시 전용이 되면서 종목 선정 자체가 없어졌고, 고객
데이터도 쓰지 않는다 — `stock_codes`를 보내도 무시되는 게 아니라 **받는 필드가 없다**.
지표는 FRED(미국)와 ECOS(한국)를 `market.py`가 직접 조회한다.
LLM이 문장을 쓰는 자리는 **밤사이 헤드라인 하나뿐**이고, 사건 수만큼(최대 3줄) 선다.
...
`Next Best Action` 채팅**(고객이 붙어 답이 키워드로 나온다). 입력 가드(MNPI·
...
1. **공개데이터 온리(사실 근거)** — 사실의 근거는 DART·공개 뉴스·공개 시세만. 유료 컨센서스나
   사내 리서치 DB는 쓰지 않는다. **고객 보유 데이터는 "무엇을 먼저 볼지" 고르는 데만** 쓰고
   (F2 유니버스), 에이전트에게는 종목코드만 넘어간다 — 산출물에 고객 식별정보는 넣지 않고
   "보유 고객 N명" 같은 집계까지만 노출한다.
