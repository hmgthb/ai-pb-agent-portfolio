...
**PB가 상담 하나를 준비하는 순서 그대로다** — 오늘 시장을 보고 → 이 고객이 무엇을 들고
있는지 보고 → 무엇을 할지 정하고 → 들고 들어갈 메모를 뽑는다.

| 화면 | 무엇을 보여주나 |
| --- | --- |
| **① 상담 전 브리핑** | 거시 지표 · **3개월 추세** · **내 고객 보유 종목** 순으로 한 화면. 문장마다 출처 배지가 붙는다. |
| **② 고객 포트폴리오** | 보유 종목·비중·자산배분과 위험 플래그. **금액이 아니라 비중(%)으로** 넘어온 값이다 (`redact.py`). |
| **③ Next Best Action** | 상담 이력·계좌 데이터를 근거로 답하고 **「다음 행동」**까지 쓴다. 출처가 확인 안 된 문장은 `[UNSOURCED]`로 표시되고 게이트 위반이 감사로그에 남는다. |
| **④ 상담 준비 메모 (PDF)** | 위 셋을 한 장으로 뽑는다. **`내부 참고용 · 고객 제공용 아님`**이 종이에 찍혀 나간다. |

<!-- 스크린샷을 docs/screenshots/에 넣은 뒤(파일명은 그 폴더의 README 참조) 아래 표로 위 표를 갈아끼운다:

| ① 상담 전 브리핑 | ② 고객 포트폴리오 |
| --- | --- |
| ![브리핑](docs/screenshots/01-brief.png) | ![고객 포트폴리오](docs/screenshots/02-portfolio.png) |
| 거시 지표 · **3개월 추세** · **내 고객 보유 종목** 순으로 한 화면. 문장마다 출처 배지가 붙는다. | 보유 종목·비중·자산배분과 위험 플래그. **금액이 아니라 비중(%)으로** 넘어온 값이다 (`redact.py`). |

| ③ Next Best Action | ④ 상담 준비 메모 (PDF) |
| --- | --- |
| ![Next Best Action](docs/screenshots/03-nba.png) | ![상담 준비 메모](docs/screenshots/04-prep-memo.png) |
| 상담 이력·계좌 데이터를 근거로 답하고 **「다음 행동」**까지 쓴다. 출처가 확인 안 된 문장은 `[UNSOURCED]`로 표시되고 게이트 위반이 감사로그에 남는다. | 위 셋을 한 장으로 뽑는다. **`내부 참고용 · 고객 제공용 아님`**이 종이에 찍혀 나간다. |
-->
