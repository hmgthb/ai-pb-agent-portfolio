...
**PB가 고객을 만나기 전에 확인할 사실**을 공개 데이터(DART 전자공시 · 공개 뉴스 · 지연시세 ·
FRED · 한국은행 ECOS)만으로 모아주는 사내 워크벤치. **오늘 거시**(지수 · 환율 · 금리 · 밤사이
헤드라인)와 **내 고객의 사정**(보유·상황·상담 이력)을 같은 화면에 놓고, 여러 Claude 에이전트가
나눠 조사해 모든 문장에 출처를 붙인다.
...
| **F2 브리핑**               | 동작   | 거시 지표 + **3개월 추세 1~2줄** + **담당 고객 관련 종목 2줄** (**새 에이전트 0개**)             |
| **F1 Next Best Action**     | 동작   | 상담 중 질문 → 규칙 라우팅 → 분석 + **「다음 행동」 절**, 문장마다 출처(멀티턴)                  |
| **H1 대시보드**             | 동작   | 브리핑 · 고객 카드(Next Best Action) · 상담 준비 메모 / (관리자) 작성·검토 · 감시               |
| F4 피어·섹터 · F5 규정 확인 | 로드맵 | **화면에 없다** — 기능 레일을 걷어내면서 자리표시도 같이 뺐다                                   |
