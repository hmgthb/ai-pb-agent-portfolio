...
계좌 숫자만으로는 상담이 안 된다. 투자성향이 `공격투자형`이어도 반년 뒤 전세 보증금을
치러야 하면 지금 자금성향은 `안정형`이고, 그 간극이 상담에서 가장 먼저 확인할 것인데 지금까지
...
⚠️ `effective_risk`는 **`f1.RISK_LABELS`의 인덱스**다(투자성향과 같은 축이라야 견줄 수 있다).
...
# 위험성향 라벨 — `f1.RISK_LABELS`와 **같은 순서여야 한다**(어긋나면 자금성향이 거짓말한다).
...
# `risk_shift` — 투자성향에서 몇 칸 옮긴 것이 **지금의 자금성향**인가.
#     음수 = 단기 자금이 묶여 보수적으로 봐야 한다 · 0 = 투자성향 그대로 ·
...
def _build(c: dict, arche: dict) -> dict:
    ...
    return {
        ...
        "plan": built["plan"],
        # 투자성향과 **같은 축**(RISK_LABELS 인덱스)이라야 화면이 둘을 견줄 수 있다.
...
def history_for(c: dict, scenario: dict) -> list[dict]:
    """상담 이력. 첫 줄의 성향 등록에는 **그때 등록한 라벨**을 적는다 — 지금 투자성향과 같지만,
