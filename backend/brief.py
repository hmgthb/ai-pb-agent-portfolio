...
# 지표별 기사 필터 — **검색어만으로는 다른 시장 기사가 섞인다**(2026-08-11 실측).
#
# `미국 국채 금리`로 찾은 결과에 한국 기사가 들어왔고, 모델이 그 제목에서 사유를 가져다
# 미국 30년물 문장에 붙였다:
#     문장: `미국채30년 금리가 … **중동리스크·환율상승** 속 19년 만에 최고 …`
#     각주[3]: `중동리스크·환율상승…7월 **국고채** 금리 전반적으로 상승`  ← 한국 국고채다
# 두 나라 채권시장이 한 문장에서 섞였다. 검색어를 고쳐도 안 되는 이유는 네이버 뉴스가
# **제목 매칭**이라 `국채`가 든 한국 기사를 계속 준다는 것이다.
#
# `{지표: (있어야 하는 말들, 있으면 버리는 말들)}` — 있어야 하는 쪽은 **OR**이고,
# 버리는 쪽이 **이긴다**(둘 다 걸리면 버린다).
# ⚠️ **이 저장소가 정한 표다**(`f1.SECTORS`·`HEADLINE_TOPICS`와 같은 성격). 공급자가 준
#    분류가 아니므로, 지표를 늘리면 여기도 같이 늘려야 한다 — 빠뜨리면 그 지표는 아무것도
#    안 걸러진다(`trend_news_filter`가 표에 없는 지표를 통과시킨다).
# ⚠️ **놓치는 쪽으로 기운다.** 관련 기사를 몇 건 버리는 것보다 다른 시장 기사를 근거로
#    삼는 편이 나쁘다 — 기사가 하나도 안 남으면 줄은 움직임만 말하고 각주 없이 선다.
# ⚠️ `국고채`에는 `국채`가 부분문자열로 들어 있지 않다(국-고-채 · 국-채) — 그래서
#    미국채 쪽 `국채`가 한국 국고채 기사를 끌어오지 않는다. 그래도 버리는 쪽에 `국고채`를
#    둔 것은 제목에 둘 다 나오는 기사를 위해서다.
TREND_NEWS_FILTERS: dict[str, tuple[tuple[str, ...], tuple[str, ...]]] = {
    "나스닥": (("나스닥",), ("코스닥",)),
    "S&P500": (("s&p", "에스앤피", "스탠더드앤드푸어스"), ()),
    "원/달러": (("환율", "원달러", "원·달러"), ()),
    "국고채10년": (("국고채",), ("미국", "美", "연준", "fed")),
    "미국채30년": (("미국", "美", "연준", "fed", "국채"), ("국고채", "한국은행", "기재부")),
}


def trend_news_filter(index_name: str, rows: list[dict]) -> list[dict]:
    """그 지표의 기사만 남긴다(순수). 표에 없는 지표는 **그대로 통과**한다.

    제목만 본다 — 본문은 애초에 안 가져온다(가드레일 5: 전송받은 텍스트는 신뢰하지 않는다).
    """
    rule = TREND_NEWS_FILTERS.get(index_name)
    if not rule:
        return list(rows or [])
    must, deny = rule
    out = []
    for r in rows or []:
        ...
        if any(d.lower() in title for d in deny):
            continue
        if must and not any(m.lower() in title for m in must):
            continue
        out.append(r)
    return out
