...
def split_labeled(raw: str) -> list[tuple[list[str] | None, str]]:
    ...
    ⚠️ 하나도 통과 못 하면 **이름표를 떼지 않고 줄째로 둔다.** 떼어 버리면 화면에서도
       게이트에서도 사라지는데, 규칙을 어긴 이름표야말로 사람 눈에 띄어야 한다.
    """
    ...
    for raw_line in (raw or "").split("\n"):
        ...
        labels = [
            part
            for raw_part in m.group("label").split(LABEL_SEP)
            if valid_label(part := raw_part.strip(), text)
        ][:LABEL_MAX_COUNT]
        out.append((labels, text) if labels else (None, line))
    return out
...
- 키워드는 **그 문장에서 그대로 떼어 온 말**이다. 갈래 이름(`상황 요약`, `성향 대비`)을
  붙이지 마라 — 그건 문장에 없는 말이다.
