...
# ⚠️ **"내부 참고용"도 뺐다**(2026-08-11 · 제품 결정). 한 문장에 두 말이 겹쳐 있었는데
#    ① 이 산출물의 용도(내부용) ② 근거의 출처(내부 계좌데이터) — 남긴 것은 ②다.
#    **딸려 나간 것 둘, 둘 다 손봤다:**
#      · `citations._BOILERPLATE_RE`가 `내부\s*참고용`을 고지문 서명으로 쓰고 있었다.
#        그대로 두면 이 고지 자체가 **출처 없는 문장**으로 게이트에 올라간다 — 그쪽에
#        `공개데이터가 아니다` 갈래를 더했다(`test_the_f2_notice_never_trips_its_own_gate`).
#      · CLAUDE.md 「기능별 필수 고지문구」 표의 F2 행도 같이 고쳤다(그 표는 여기와 1:1이다).
#    ⚠️ 노트 PDF·상담 메모 PDF의 `내부 참고용 · 고객 제공용 아님`은 **그대로다**(notepdf.py).
#       그건 종이에 찍혀 나가는 문서의 용도 표시이고, 이 값과 다른 자리다.
BRIEF_NOTICE = "ℹ 고객 포트폴리오는 내부 계좌데이터로 공개데이터가 아닙니다."
