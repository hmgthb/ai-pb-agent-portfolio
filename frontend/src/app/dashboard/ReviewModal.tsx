 ...
 *  4 PB 제거: **맨 아래**다(2026-08-03). 관리자 확인 셀렉트가 잠겨 있어 이 화면에서 할 일이
          ...
          <div className="srow" key={i}>
            ...
            <span className="stext">
              ...
              {/* PB 판정은 위 배지를 **대신하지 않고 뒤에 붙는다** — UNSOURCED·해석은
                  ...
                  미인용이라는 상태 자체가 풀린다.) */}
              {/* 출처 있는 문장을 관리자가 뺀 경우 — 미인용 배지 자리가 없으므로 여기서
                  따로 단다. 미인용 문장은 위 블록이 이미 `관리자 제거`를 그린다. */}
            ...
            {/* **`PB 승인` 문장에만 열린다(2026-08-03).** 관리자가 여는 게이트는 "이대로
            ...
            {/* 출처 있는 문장에 관리자가 쓰는 조작은 **`제거` 하나뿐**이다. 확인(사유)은
                ...
                문장에서 나면, 뺄 길이 없는 한 관리자에게 남는 선택지가 반려뿐이었다. */}
            ...
            {/* 금지 표현 예외 — **이 앱에서 유일하게 사유를 손으로 적는 자리**다.
                ...
                ⚠️ 규칙을 여는 조작이므로 **심의 단계의 관리자에게만** 그린다(canAck과 같은 조건).
...
export function NoteModal({
  ...
  /* 사실 확인은 노트를 만든 쪽이 하고, 마지막 단계(발행)만 다른 쪽에 남긴다:
     **만드는 사람과 통과시키는 사람이 갈리는 지점이 여기 하나**이고, 그게 이 제품의 핵심이다.
     그래서 심의중 노트는 권한이 없는 쪽에서 상태만 보이고 버튼이 없다(아래 deny 문구가 이유).

     ⚠️ **2026-08-10에 이 분리가 화면에서는 흐려졌다.** 노트를 만드는 「작성·검토」가 관리자
        화면으로 옮겨 가면서, 만드는 쪽과 통과시키는 쪽이 같은 화면이 됐다. 권한 판정
        (`role === 'comp'`)은 그대로 두었으므로 **코드는 아직 둘을 가른다** — 되돌릴 때
        건드릴 곳도 여기가 아니라 page.tsx의 탭 배치다. */
  ...
  /* 각 단계마다 **앞으로 가는 길과 거절하는 길이 같이 있다.** 거절이 없으면 이 화면은
     ...
     **반려**(심의중, 관리자)는 고쳐서 다시 올리라는 되돌림이다. */
  ...
  /* 단계별 담당자 줄("확인 PB · 심의 요청 PB · 발행 —")은 뺐다(2026-07-28).
     역할이 `PB`/`관리자` 둘뿐이라 어느 노트에서나 같은 값이 나왔고, "어디까지 갔나"는
     제목 옆 상태 알약이 이미 말한다. **누가 언제 했는지는 감사로그가 정본**이고
     관리자·감시 탭의 노트별 조회에서 전건을 본다(모달에 원장을 복제하지 않는다).
  ...
  /* 각주를 붙일 수 없는 문장(해석·고지·데이터 설명)이 실제로 있어서, 게이트가 그걸 전부
     잠그면 사람이 열 방법이 없다 — 그래서 관리자가 사유를 적어 확인하면 미인용 집계에서
     빠진다. 확인은 **심의 단계에서만**(검토 단계에서 미리 풀면 검토가 형식이 된다) 그리고
     발행과 같은 권한(관리자)에게만 연다. */
  ...
  /* 그중 **이 화면에서 풀 수 없는 것** = PB 판정이 없는 문장. 확인 셀렉트가 `PB 승인`
     문장에만 열리므로 관리자가 손댈 수 없는데 게이트는 그대로 센다 — 몇 개가 그런지
  ...
  /* PB 판정 — 노트가 아직 PB 손에 있는 동안(검토중) 각주 없는 문장을 훑는다.
     ...
     ⚠️ 이 판정은 **게이트를 열지 않는다**(backend PB_MARKS 주석) — 미인용 문장을
        발행 가능하게 만드는 건 관리자의 확인뿐이다. 여기서 바뀌는 건 표시와 감사로그다.
     ⚠️ 다만 관리자의 확인 셀렉트는 `PB 승인` 문장에만 열리므로(위 SentenceRows 주석),
        **여기서 판정하지 않고 올린 문장은 관리자가 풀 수 없다.** 여기가 그 단계다. */
    ...
    <>
      ...
      <div className="m-body">
        ...
        {/* 발행분에서만 서는 조작 — 이 상태의 노트에는 상태 전이가 없어서 조작 줄이
            비어 있었다. **PB·관리자 둘 다에게 낸다**: 통과시키는 사람은 관리자이지만 상담에
        ...
        {/* 감사로그는 여기 두지 않는다 — 관리자 · 감시 탭의 감사로그 카드에서 노트별로
...
export function ChatModal({
   ...
   *  관리자에게는 고객 포트폴리오 카드 자체가 없으므로 넘어오지 않는다 — 정보장벽. */
  ...
  return (
    <>
      ...
      <div className="m-body">
        ...
        {/* ── 여기부터가 "회신 전에 확인할 사실" ──────────────────────────
            담당 PB가 아니면 그리지 않는다. 관리자에게 고객 보유·공시가 갈 이유가 없고
