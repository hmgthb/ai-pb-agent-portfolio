 ...
 *  여는 건 결국 노트였다.
 *
 *  ⚠️ **공시·뉴스 줄은 지금 뜨지 않는다**(2026-08-07). 그 재료는 브리프의 `items`에서 왔는데,
 *     브리핑이 거시 전용이 되면서 `items`가 항상 빈 배열이다(types.ts `BriefItem`) — 그래서
 *     아래 `b`는 늘 null이고 남는 줄은 **노트 하나**다.
 *     ⚠️ 아래 고르기 규칙(임원 보고 제외 · 공시 2 · 뉴스 1)을 **지우지 말 것.** 종목 수집이
 *        다른 카드로 살아나면 그 결과를 `brief` 자리에 그대로 꽂으면 된다. 규칙을 지웠다가
 *        다시 쓰면 브리핑 카드와 여기가 다른 기준으로 같은 종목을 보여주게 된다.
    ...
    <>
      ...
      {/* 노트 줄은 **발행분에만** 선다(2026-08-06). 이 자리에서 여는 건 검토 화면이 아니라
          **최종본 PDF**이고, 그건 발행된 노트에만 있다 — 상태 알약·날짜를 곁들이지 않는 것도
          ...
          검토·심의 중인 노트를 여는 길은 **관리자 화면**의 「작성·검토」 큐다
          (2026-08-10에 PB에서 옮겼다 — 이 화면에서는 갈 수 없다). */}
      {note && (
      ...
      {/* ⚠️ 문구에서 `브리핑`을 뺐다(2026-08-07) — 브리핑에 종목이 없으므로, 거기 이 종목이
             있고 없고를 말하면 화면에 없는 기준을 근거로 드는 셈이다. */}
      {!lines.length && !note && (
