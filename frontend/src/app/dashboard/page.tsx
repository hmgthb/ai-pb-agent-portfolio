...
export default function DashboardPage() {
  ...
  return (
    <div className="wrap">
                ...
                <div className="detail">
                  {selected && (
                    <>
                      ...
                      {/* **투자성향** 옆에 **자금성향**(상황을 반영한 위험성향)을
                          붙인다(2026-08-07 · 이름은 2026-08-10에 `실질 성향`에서 바뀌었다).
                          이 카드가 오래 `공격투자형`만 적어 왔는데, 정작 상담에서 먼저 확인할
                          것은 그 투자성향이 지금도 유효한가다 — 반년 뒤 보증금을 치러야 하면
                          투자성향이 공격투자형이어도 지금 자금성향은 그렇지 않다.
                          ⚠️ **다를 때만 낸다.** 같은데도 `= 자금성향 …`을 적으면 매 고객 줄에
      ...
      {/* ── 페이지를 닫는 띠 ─────────────────
          한때 여기 화면 전체에 걸리는 고지가 있었다(2026-08-10에 걷어냈다 · 아래 주석).
          **개별 산출물은 각자 자기 고지를 그대로 달고 다닌다** — 종목 노트=워터마크
          (ReviewModal) · F1 답변=지연시세·내부 계좌데이터 고지(F1Chat) · 브리핑=백엔드가
          `briefs.content_md`에 넣은 "ℹ 내부 참고용"(게이트가 검사한다).
          ⚠️ 화면 타입(Brief)에 content_md 필드가 없는 건 누락이 아니라 그 결과다 —
             "브리핑에 고지가 없다"고 다시 열지 말 것(HANDOFF 열린 항목에서도 지웠다). */}
      <footer className="page-footer">
        {/* ⚠️ **화면 하단 고지문을 걷어냈다**(2026-08-10). 이 자리에 "내부 참고용 미검증
            초안이며 투자권유·광고가 아닙니다"가 있었는데, 이 도구가 PB에게 종목을 추천하는
            쪽으로 방향을 바꾸면서 화면이 하지 않는 말을 하고 있게 됐다.
            ⚠️ **없어진 것은 화면 하단의 문장 하나뿐이다.** 산출물에 붙는 기능별 고지
               (`compliance.NOTICES`)는 그대로 살아 있고, 지연시세 명시·내부 계좌데이터
               명시도 그대로다 — 되살린다면 그 둘과 겹치지 않게 쓸 것.
            ⚠️ 푸터 자체는 남긴다. 아래 톱니가 여기 살고, 그게 관리자 화면으로 가는
               **유일한 문**이다. */}
        {/* 관리자 화면으로 가는 문 — **푸터 오른쪽 끝에 선 흐린 톱니 하나**(2026-08-10).
            한때 이 아래에 줄 하나짜리 푸터를 따로 세웠는데, 페이지를 닫는 띠가 둘이 되면서
            그 사이에 구분선만 뜬 빈 칸이 생겼다 — 문 하나 때문에 페이지의 마지막 면을
            늘릴 이유가 없다.
            ...
            ⚠️ 고지문이 빠지면서 푸터에 이것만 남았다. `.page-footer`가 오른쪽 정렬을
               유지해야 자리가 안 바뀐다(dashboard.css) — **아는 사람이 반드시 찾는 자리**인
               것이 이 문의 전제다. */}
        {role === 'pb' && (
